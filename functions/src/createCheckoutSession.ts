...
import { Product, Price, Subscription, CustomerData, TaxRate } from './interfaces';
...
const createCustomerRecord = async ({ email, uid }: { email?: string; uid: string }) => {
  try {
    ...
    await admin.firestore().collection(config.customersCollectionPath).doc(uid).set(customerRecord, { merge: true });
...
exports.createCustomer = functions.auth.user().onCreate(async (user): Promise<void> => {
...
exports.createCheckoutSession = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      if (client_reference_id) sessionCreateParams.client_reference_id = client_reference_id;
...
exports.createPortalLink = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // Get stripe customer id
    const customer = (await admin.firestore().collection(config.customersCollectionPath).doc(uid).get()).data()
      .stripeId;
...
const createProductRecord = async (product: Stripe.Product): Promise<void> => {
  ...
  await admin.firestore().collection(config.productsCollectionPath).doc(product.id).set(productData);
 ...
 */
export const handleWebhookEvents = functions.handler.https.onRequest(async (req: functions.https.Request, resp) => {
...
export const onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  const customer = (await admin.firestore().collection(config.customersCollectionPath).doc(user.uid).get()).data();
