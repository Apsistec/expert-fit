...
const createCustomerRecord = async ({
  ...
  try {
    ...
    await admin
        .firestore()
        .collection(config.customersCollectionPath as any)
        .doc(uid)
...
/**
 * Create a CheckoutSession for the customer so they can sign up for the sub
 */
...
const deleteStripeCustomer = async ({
  ...
  try {
    ...
    const subscriptionsSnap = await admin
        ...
        .get();
    subscriptionsSnap.forEach(async (doc) => {
